...
为了解当前效果所处的位置，我们还在相同的三个全拼集合上测试了最新版雾凇拼音配合
万象 LTS 语言模型。两边均使用空用户词库，不加载历史调频，也不在评测过程中提交
候选。

| 输入方案 | 整句准确率 | 字准确率 |
|---|---:|---:|
| Sime GRU | **64.74%**（3466/5354） | **91.89%** |
| 雾凇拼音 + 万象 LTS | 59.25%（3172/5354） | 89.08% |

这只是特定测试集合和固定配置下的参考结果，并不代表所有输入场景中的体验。不同方案
